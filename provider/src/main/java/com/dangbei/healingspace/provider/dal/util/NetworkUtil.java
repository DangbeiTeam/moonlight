...
public class NetworkUtil {
    ...
    public static boolean isConnected(Context context) {
        ...
        try {
            ...
            return net != null && net.isConnected() && net.isAvailable();
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    public static boolean isEtherent(Context context) {
        NetworkInfo[] nets = getConnectivityManager(context).getAllNetworkInfo();
        if (nets != null) {
            for (NetworkInfo net : nets) {
                if (net.getType() == ConnectivityManager.TYPE_ETHERNET) {
                    return net.isAvailable() && net.isConnected();
                }
            }
        }
        return false;
...
//            L.i("getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
//                    L.i("NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
//                    L.i("NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
//                    L.i("NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
//                    L.i("NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
//                L.i("\n");
            } else {
//                L.i("getAllNetworkInfo is null");
            }
        }
        return false;
    }
